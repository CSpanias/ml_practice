"""
Guided script from the book "Machine Learning for Absolute Beginners
"""
...
# load dataset
df = pd.read_csv("C:\\Users\\10inm\\Desktop\\ml_practice\\general_datasets\\Melbourne_housing_FULL.csv")
...
model = ensemble.GradientBoostingRegressor(
        ...
        learning_rate=0.1, #
        max_depth=6, # max number of layers/tree
        min_samples_split=4, # min number of samples required to do a new split
        min_samples_leaf=6, # min number of samples that must have each node
        max_features=0.6, # 
        loss='huber', # calculates the error rate
) 
...
# Check model's accuracy up to two decimal places
#mae_train = mean_absolute_error(y_train, y_pred_train)
#mae_test = mean_absolute_error(y_test, y_pred)
#print("Training Set Mean Absolute Error: %.2f $." % mae_train)
#print("Testing Set Mean Absolute Error: %.2f $." % mae_test)

# generate a list with all Xs

# convert dataframe columns in a list
...
model_prediction = model.predict([house_to_value])
